...
export const Navbar: React.FC = () => {
  ...
  // Determine if we're in host mode based on current route
  ...
  return createPortal(
    <>
      <nav className="fixed top-8 left-4 md:left-8 z-[2000] flex items-center gap-0">
        ...
        {/* Role Toggle - Desktop (only show HOST if admin) */}
        <div className="hidden md:flex items-center border border-l-0 border-black h-[34px] bg-white overflow-hidden">
          <Link
            to="/"
            className={`relative h-full px-3 flex items-center text-[11px] font-medium uppercase leading-none transition-colors ${
              !isHostMode ? 'bg-[#FA76FF] text-black' : 'bg-white text-black hover:bg-black/5'
            }`}
          >
            ATTENDEE
          </Link>
          {isAdmin && (
            <Link
              to="/host"
              className={`relative h-full px-3 flex items-center text-[11px] font-medium uppercase leading-none border-l border-black transition-colors ${
                isHostMode ? 'bg-[#FA76FF] text-black' : 'bg-white text-black hover:bg-black/5'
              }`}
            >
              HOST
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden fixed inset-0 z-[3000] flex flex-col animate-in slide-in-from-top duration-300">
            ...
            {/* Role Toggle - only show HOST if admin */}
            <div className="flex border-b border-black">
              <Link 
                to="/"
                onClick={() => setIsMobileMenuOpen(false)}
                className={`flex-1 py-4 text-center text-[13px] font-medium uppercase ${
                  !isHostMode ? 'bg-[#FA76FF]' : 'bg-white'
                }`}
              >
                ATTENDEE
              </Link>
              {isAdmin && (
                <Link 
                  to="/host"
                  onClick={() => setIsMobileMenuOpen(false)}
                  className={`flex-1 py-4 text-center text-[13px] font-medium uppercase border-l border-black ${
                    isHostMode ? 'bg-[#FA76FF]' : 'bg-white'
                  }`}
                >
                  HOST
